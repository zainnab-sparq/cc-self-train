...
A burst of plugin-layer polish worth skimming once you're comfortable with the basics:
...
- **Managed-settings enforcement.** `blockedMarketplaces` and `strictKnownMarketplaces` are enforced at plugin install time (v2.1.117); `blockedMarketplaces` now correctly honors both `hostPattern` and `pathPattern` (v2.1.119). Use in managed settings for locked-down environments.
- **`DISABLE_UPDATES`** (v2.1.115). Env var that blocks *every* update path — `claude update`, background checks, plugin auto-updates. Stricter than `CLAUDE_CODE_DISABLE_AUTOUPDATER` (which only blocks automatic upgrades). Appropriate for managed environments that mandate a specific CC version.

None of this requires action for Canvas. Surface-level awareness so when you hit a weird plugin state, you know where to look.
