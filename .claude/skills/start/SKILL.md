...
### `/start` interrupted or crashed

If the user ran `/start` before but didn't finish (Ctrl+C, closed terminal, Claude errored out):

1. Check for `.claude/onboarding-state.json` — if it exists with valid data, resume from the saved step (Step 3b handles this automatically).
2. If the state file is missing or corrupt, tell the user: "Looks like your previous session didn't save its progress. No worries — let's start fresh." Then begin from Step 1.
3. If `CLAUDE.local.md` already exists (they finished `/start` before), tell them: "You've already completed onboarding! Say **'next module'** to continue where you left off."

### Workspace directory already exists

If `workspace/<project>/` already exists when reaching Step 5:

- **If it contains files with git history:** Ask the user — "I see you already have a `<project>` directory with work in it. Want to continue with it, or start fresh?" If they want fresh, rename the old one: `mv workspace/<project> workspace/<project>-backup-$(date +%s)`.
- **If it's empty or has no git history:** Reuse it silently — just run `git init` if needed.
- **Never delete existing work without asking.**

### Git not configured

If `git config user.name` or `git config user.email` returns empty in Step 6.5:

- This is handled by Step 6.5, but if git itself isn't installed and Phase C couldn't install it, tell the user: "Git is required for this course. Install it from [git-scm.com](https://git-scm.com/downloads), restart your terminal, then run `/start` again — I'll pick up where you left off."

### Permission denied errors

- **On Linux/macOS:** If a command fails with "permission denied", check if `sudo` is needed. Explain: "This command needs admin access. You may see a password prompt." Never run `sudo` silently.
- **On Windows:** If a command fails with access errors, suggest running the terminal as Administrator. If they're in VS Code, the integrated terminal inherits VS Code's permissions.
- **File write failures:** If creating files in `workspace/` fails, check directory permissions. The `workspace/` directory should be writable — if it's not, suggest: "Try creating the directory manually and re-run `/start`."

### Tool installation failures

Phase C already handles most cases, but if everything fails:

1. Show the manual download URL for the tool
2. Give step-by-step manual install instructions
3. Tell the user: "After installing, close Claude Code (`Ctrl+D`), reopen it (`claude`), and run `/start` again — I'll resume from where you left off."
4. Never block onboarding indefinitely — if a non-critical tool can't be installed, note it and continue.

### Browser won't open (Canvas)

If the browser command fails in Step 5 (common on WSL, headless servers, or remote machines):

- Tell the user the file path and how to open it manually
- For WSL users: "You're running in WSL, which can't open Windows browsers directly. Navigate to `\\wsl$\<distro>\<path>\index.html` in Windows File Explorer, or copy the file to a Windows directory."

### State file issues

- **Corrupt `.claude/onboarding-state.json`:** If JSON parsing fails, delete the file and restart from Step 1. Tell the user what happened.
- **State file says Step 6 but workspace is missing:** The user may have deleted their workspace. Re-run Step 5 (scaffold) before continuing to Step 6.
- **State file references a different project than expected:** Ask the user: "Your saved progress is for **[project]**, but it looks like something changed. Want to continue with [project], or start over with a different one?"

### Network failures

- **Curriculum sync fails:** Already handled by the "Graceful failure" section — continue to Step 1b.
- **WebSearch fails in Step 6.6:** Fall back to reading `context/interactive-mode.txt` for keyboard shortcuts. Tell the user: "I couldn't search the web right now, but I have a local reference file with the shortcuts."

### General recovery principle

**If something goes wrong and you're not sure how to fix it:**

1. Tell the user what happened (no jargon — plain language)
2. Save current progress to `.claude/onboarding-state.json`
3. Suggest: "Close Claude Code (`Ctrl+D`), fix the issue, then reopen and run `/start` — your progress is saved."
4. Never silently swallow errors — the user should always know what went wrong and what to do next.
